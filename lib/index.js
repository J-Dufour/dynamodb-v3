...
dynamo.dynamoDriver = internals.dynamoDriver = function (driver) {
  if(driver) {
    ...
    internals.dynamodb = internals.dynamodb || new DynamoDBClient();
  }
...
dynamo.documentClient = internals.documentClient = function (docClient) {
  if(docClient) {
    ...
    // In v3, get the underlying client from the document client's config
    internals.dynamodb = docClient.config.client;
    internals.updateDynamoDBDocClientForAllModels(docClient);
  } else {
...
internals.updateDynamoDBDocClientForAllModels = function (docClient) {
  _.each(dynamo.models, function (model) {
    // Set both the document client and the underlying base client
    model.config({
      docClient: docClient,
      dynamodb: internals.dynamodb
    });
  });
};
...
internals.loadDocClient = function (driver) {
  if(driver) {
    internals.docClient = DynamoDBDocumentClient.from(driver);
  } else {
    internals.docClient = internals.docClient || DynamoDBDocumentClient.from(internals.dynamoDriver());
  }
...
internals.compileModel = function (name, schema) {
  ...
  var table = new Table(tableName, schema, serializer, internals.loadDocClient(), internals.dynamoDriver(), log);
  ...
  Model.config = function(config) {
    ...
    if (config.docClient) {
      ...
      // Also set the dynamoDbClient if it's provided
      if (config.dynamodb) {
        table.dynamoDbClient = config.dynamodb;
      }
    } else if (config.dynamodb) {
      table.docClient = DynamoDBDocumentClient.from(config.dynamodb);
      table.dynamoDbClient = config.dynamodb;
    }
...
dynamo.Set = function () {
  // In v3, JavaScript Sets are automatically marshalled to DynamoDB sets
  // Set<String> → SS, Set<Number> → NS, Set<Uint8Array, Blob> → BS
  
  // Handle v2-style calls with type parameter: dynamo.Set(['a', 'b'], 'S')
