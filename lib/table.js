...
var Table = module.exports = function (name, schema, serializer, docClient, dynamoDbClient, logger) {
  this.config = {name : name};
  this.schema = schema;
  this.serializer = serializer;
  this.docClient = docClient;  // Shared DynamoDBDocumentClient for item operations
  this.dynamoDbClient = dynamoDbClient;  // Base DynamoDBClient for table management operations
  this.log = logger;
...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  try {
    var command = self.createCommand(method, params);
    var client = self.getClient(method);
    ...
    client.send(command).then(function(data) {
      ...
      var elapsed = Date.now() - startTime;
      self.log.warn({err : err}, 'dynamo %s error - %sms', method.toUpperCase(), elapsed);
      
      // Normalize error for backward compatibility
      // In AWS SDK v3, error codes are in the 'name' property
...
Table.prototype.createCommand = function (method, params) {
  var commandMap = {
    'get': GetCommand,
    'put': PutCommand,
  ...
  var CommandClass = commandMap[method];
  if (!CommandClass) {
...
Table.prototype.getClient = function (method) {
  // Document operations use docClient
  var docOperations = ['get', 'put', 'update', 'delete', 'scan', 'query', 'batchGet', 'batchWrite'];
  
  if (docOperations.indexOf(method) !== -1) {
    return this.docClient;
  } else {
    // Table management operations use raw DynamoDB client
