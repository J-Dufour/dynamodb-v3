...
var serialize = internals.serialize = {
  ...
  stringSet : function (value) {
    // v3 automatically detects Set<String> → SS
    return Array.isArray(value) ? new Set(value) : new Set([value]);
  },

  numberSet : function (value) {
    // v3 automatically detects Set<Number> → NS  
    return Array.isArray(value) ? new Set(value) : new Set([value]);
  },

  binarySet : function (value) {
    // v3 automatically detects Set<Uint8Array/Buffer> → BS
    var bins = value;
    if(!_.isArray(value)) {
      bins = [value];
    }
...
internals.deserializeAttribute = function (value) {
  if(_.isObject(value) && _.isFunction(value.detectType) && _.isArray(value.values)) {
    // value is a Set object from document client (v2 style)
    return value.values;
  } else if (value instanceof Set) {
    // In v3, Sets are native JavaScript Sets - convert to array
